...
func add_node(node: NodeGraph): 
	if node not in nodes:
		nodes.append(node)
		update_connections()
		
func remove_node(node: NodeGraph):
	if node in nodes:
		nodes.erase(node)
		for other in nodes:
...
func update_connections():
	...
	for i in range(nodes.size()):
		for j in range(i + 1, nodes.size()):
			var node_a = nodes[i]
			var node_b = nodes[j]
			if node_a.global_position.distance_to(node_b.global_position) <= node_a.get_node("ChainRadius").get_node("CollisionShape2D").shape.get_radius() + node_b.get_node("ChainRadius").get_node("CollisionShape2D").shape.get_radius():
